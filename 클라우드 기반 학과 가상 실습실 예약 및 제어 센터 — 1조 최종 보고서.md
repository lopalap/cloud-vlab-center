...
본 테스트는 화이트박스 4가지 기법과 블랙박스 4가지 기법을 코드 특성에 맞게 적용하여 유효성을 검증하였다.

| 기법명 | 분류 | 선정 근거 |
|---|---|---|
| **기초 경로** | 화이트박스 | 프로그램 논리 경로 기반 |
| **조건 검사** | 화이트박스 | 논리 조건(T/F) 분기 기반 |
| **루프 검사** | 화이트박스 | 반복문 실행 횟수 기반 |
| **데이터 흐름** | 화이트박스 | 변수 정의/사용 추적 기반 |
| **동등 분할** | 블랙박스 | 입력값 유효/무효 영역 분할 |
| **경계값 분석** | 블랙박스 | 입력 조건 경계값 기반 |
| **원인-결과** | 블랙박스 | 입력-출력 관계 분석 기반 |
| **오류 예측** | 블랙박스 | 경험 기반 오류 추측 |

---

### 5-2. 상세 테스트 케이스 및 수행 결과

#### A. 인증 테스트 (적용 기법: 동등 분할, 조건 검사, 경계값 분석, 기초 경로, 원인-결과)

* **이메일 인증 코드 발송 및 검증**
    | TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
    |---|---|---|---|---|---|---|---|
    | TC-001 | 동등 분할 | 유효 이메일 코드 발송 성공 | 서버 실행 중 | 유효한 이메일<br>(user@gmail.com) | 200 OK, '인증 코드가 발송되었습니다.' | ‘서버 오류’ | **F** |
    | TC-002 | 동등 분할 | 무효 이메일 형식 발송 실패 | 서버 실행 중 | 형식 오류 이메일<br>(user@) | 400, '올바른 이메일 형식이 아닙니다.' | 400, '올바른 이메일 형식이 아닙니다.' | **P** |
    | TC-003 | 조건 검사 | 이메일 미입력 발송 실패 | 서버 실행 중 | email 필드 빈 문자열 | 400, '이메일을 입력해주세요.' | 400, '이메일을 입력해주세요.' | **P** |
    | TC-004 | 조건 검사 | 올바른 코드 인증 성공 | 인증 코드 발송 완료 | 발송된 6자리 코드 입력 | 200 OK, verified: true 저장 | 200 OK, verified: true 저장 | **P** |
    | TC-005 | 조건 검사 | 잘못된 코드 인증 실패 | 인증 코드 발송 완료 | 틀린 6자리 코드 입력 | 400, '인증 코드가 올바르지 않습니다.' | 400, '인증 코드가 올바르지 않습니다.' | **P** |
    | TC-006 | 경계값 분석 | 만료 시점(5분 경과) 인증 실패 | 코드 발송 후 5분 경과 | 발송된 코드 입력 | 400, '인증 코드가 만료되었습니다.' | 400, '인증 코드가 만료되었습니다.' | **P** |
    | TC-007 | 조건 검사 | 코드 미요청 상태 검증 시도 | 인증 코드 발송 이력 없음 | 임의 코드와 이메일 입력 | 400, '인증 코드를 먼저 요청해주세요.' | 400, '인증 코드를 먼저 요청해주세요.' | **P** |

* **회원가입 및 로그인**
    | TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
    |---|---|---|---|---|---|---|---|
    | TC-008 | 기초 경로 | 전체 조건 정상 — 회원가입 성공 | 이메일 인증 완료<br>(verified: true) | name/student_id/<br>email/password 모두 유효 | 201 Created, DB 저장, 인증 기록 삭제 | 201 Created, DB 저장, 인증 기록 삭제 | **P** |
    | TC-009 | 기초 경로 | 이메일 인증 미완료 — 차단 | 이메일 인증 안 된 상태 | 정상 회원가입 정보 | 400, '이메일 인증이 완료되지 않았습니다.' | 400, '이메일 인증이 완료되지 않았습니다.' | **P** |
    | TC-010 | 기초 경로 | 중복 학번 — 차단 | 동일 학번 계정 존재,<br>이메일 인증 완료 | 기존과 동일한 학번 | 400, '이미 존재하는 학번입니다.' | 400, '이미 존재하는 학번입니다.' | **P** |
    | TC-011 | 경계값 분석 | 비밀번호 5자 — 실패<br>(경계값 미만) | 이메일 인증 완료 | password: '12345'<br>(5자) | 400, '비밀번호는 6자 이상이어야 합니다.' | 400, '비밀번호는 6자 이상이어야 합니다.' | **P** |
    | TC-012 | 경계값 분석 | 비밀번호 6자 — 성공<br>(경계값) | 이메일 인증 완료 | password: '123456'<br>(6자) | 201 Created, 회원가입 성공 | 201 Created, 회원가입 성공 | **P** |
    | TC-013 | 원인-결과 | 정상 로그인 — 토큰 발급 | 활성 계정 존재 | 올바른 학번 + 비밀번호 | 200 OK, accessToken + refreshToken 반환 | 200 OK, accessToken + refreshToken 반환 | **P** |
    | TC-014 | 동등 분할 | 존재하지 않는 학번 — 실패 | 해당 학번 계정 없음 | 없는 학번 입력 | 404, '존재하지 않는 사용자' | 404, '존재하지 않는 사용자' | **P** |
    | TC-015 | 동등 분할 | 잘못된 비밀번호 — 실패 | 가입된 계정 존재 | 올바른 학번 + 틀린 비밀번호 | 401, '비밀번호 불일치' | 401, '비밀번호 불일치' | **P** |
    | TC-016 | 조건 검사 | 이용정지 계정 차단 | is_active: false 계정 존재 | 이용정지 계정 학번<br>+ 비밀번호 | 403, '비활성화된 계정' | 403, '비활성화된 계정' | **P** |

#### B. 예약 테스트 (적용 기법: 동등 분할, 경계값 분석, 오류 예측, 기초 경로, 조건 검사, 원인-결과)

| TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
|---|---|---|---|---|---|---|---|
| TC-017 | 동등 분할 | 필수 필드 누락 — 차단 | 로그인 상태 | resource_id 누락 | 400, '필수입니다.' 반환 | 400, '필수입니다.' 반환 | **P** |
| TC-018 | 경계값 분석 | 과거 시간으로 예약 — 차단 | 로그인 상태, 유효 자원 | start_time = 현재보다 1분 이전 | 400, '과거 시간으로는 예약할 수 없습니다.' | 400, '과거 시간으로는 예약할 수 없습니다.' | **P** |
| TC-019 | 기초 경로 | 자원 없음 — 404 | 해당 ID 자원 없음 | 존재하지 않는 resource_id | 404, '존재하지 않는 리소스입니다.' | 404, '존재하지 않는 리소스입니다.' | **P** |
| TC-020 | 기초 경로 | maintenance 자원 — 차단 | 자원 status: maintenance | 해당 자원으로 예약 | 400, '현재 점검 중인 시설입니다.' | 400, '현재 점검 중인 시설입니다.' | **P** |
| TC-021 | 조건 검사 | 운영 요일 외(토요일) — 차단 | 운영 요일: mon~fri | 토요일 날짜로 예약 | 400, '운영일이 아닙니다.' | 400, '운영일이 아닙니다.' | **P** |
| TC-022 | 경계값 분석 | 운영 시작 시간 경계(09:00) 성공 | 운영 시간: 09:00~22:00 | 09:00~11:00 예약 | 201 Created | 201 Created | **P** |
| TC-023 | 경계값 분석 | 운영 시간 이전(08:59) — 차단 | 운영 시간: 09:00~22:00 | 08:59~10:00 예약 | 400, 운영 시간 외 오류 | 400, 운영 시간 외 오류 | **P** |
| TC-024 | 조건 검사 | 완전 동일 예약 중복 — 409 | 동일 자원/시간 예약 존재(waiting) | 동일 자원, 동일 시작/종료 시간 | 409, '이미 동일한 예약이 존재합니다.' | 409, '이미 동일한 예약이 존재합니다.' | **P** |
| TC-025 | 조건 검사 | 시간 겹침 예약 초과 — 409 | 자원 max_concurrent:1,<br>10~12시 예약 존재 | 11:00~13:00으로 동일 자원 예약 | 409, 최대 동시 예약 수 초과 | 409, 최대 동시 예약 수 초과 | **P** |
| TC-026 | 조건 검사 | 예약 가능 횟수 초과 — 차단 | current_reservations<br>>= max_reservations | 새 예약 신청 | 400, '예약 가능 횟수를 초과했습니다.' | 400, '예약 가능 횟수를 초과했습니다.' | **P** |
| TC-027 | 기초 경로 | 모든 검사 통과 — 예약 성공 | 로그인, 유효 자원, 정상 시간 | 유효한 모든 필드 입력 | 201 Created, status: waiting | 201 Created, status: waiting | **P** |
| TC-028 | 원인-결과 | 예약 승인 (waiting → reserved) | 관리자 로그인,<br>waiting 예약 존재 | PATCH /:id/approve | 200 OK, status: reserved,<br>approved_by 기록 | 200 OK, status: reserved,<br>approved_by 기록 | **P** |
| TC-029 | 조건 검사 | waiting 아닌 예약 승인 차단 | status: reserved 예약 | 승인 요청 | 400, '대기 중인 예약만 승인할 수 있습니다.' | 400, '대기 중인 예약만 승인할 수 있습니다.' | **P** |
| TC-030 | 원인-결과 | 예약 거절 (waiting → cancelled) | 관리자 로그인,<br>waiting 예약 존재 | PATCH /:id/reject<br>+ cancel_reason | 200 OK, status: cancelled,<br>cancel_reason 저장 | 200 OK, status: cancelled,<br>cancel_reason 저장 | **P** |
| TC-031 | 기초 경로 | 사용 시작 (reserved → using) | 로그인, reserved 예약 존재 | PATCH /:id/start | 200 OK, status: using,<br>actual_start_time 기록 | 200 OK, status: using,<br>actual_start_time 기록 | **P** |
| TC-032 | 기초 경로 | 사용 종료 (using → completed) | 로그인, using 예약 존재 | PATCH /:id/end | 200 OK, status: completed,<br>actual_end_time 기록 | 200 OK, status: completed,<br>actual_end_time 기록 | **P** |
| TC-033 | 조건 검사 | 본인 아닌 예약 취소 시도 차단 | 타인 예약 | PATCH /:id/cancel | 403, '본인 예약만 취소할 수 있습니다.' | 403, '본인 예약만 취소할 수 있습니다.' | **P** |

#### C. 사용자 관리 테스트 (적용 기법: 조건 검사, 동등 분할, 경계값 분석)

| TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
|---|---|---|---|---|---|---|---|
| TC-034 | 동등 분할 | 내 정보 조회 성공 | 로그인 상태, is_active: true | GET /users/me | 200 OK, 사용자 정보 반환<br>(비밀번호 제외) | 200 OK, 사용자 정보 반환<br>(비밀번호 제외) | **P** |
| TC-035 | 경계값 분석 | 비밀번호 5자로 수정 — 차단 | 로그인 상태 | password: '12345' | 400, '비밀번호는 6자 이상이어야 합니다.' | 400, '비밀번호는 6자 이상이어야 합니다.' | **P** |
| TC-036 | 경계값 분석 | 비밀번호 6자로 수정 — 성공 | 로그인 상태 | password: '123456' | 200 OK, 수정 완료 | 200 OK, 수정 완료 | **P** |
| TC-037 | 동등 분할 | 관리자 — 전체 사용자 목록 조회 | 관리자 로그인 | GET /users | 200 OK, 전체 목록 반환 | 200 OK, 전체 목록 반환 | **P** |
| TC-038 | 조건 검사 | 학생 권한 — 전체 사용자 목록 차단 | 학생 로그인 | GET /users | 403, '관리자 권한 필요' | 403, '관리자 권한 필요' | **P** |
| TC-039 | 동등 분할 | 이용정지 처리 (is_active: false) | 관리자 로그인, 활성 계정 존재 | PATCH /users/:id<br>+ {is_active: false} | 200 OK, is_active: false | 200 OK, is_active: false | **P** |

#### D. 인증 미들웨어 테스트 (적용 기법: 기초 경로, 조건 검사, 오류 예측)

| TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
|---|---|---|---|---|---|---|---|
| TC-040 | 기초 경로 | 토큰 없이 인증 API 호출 — 401 | 비로그인 상태 | Authorization 헤더 없이<br>GET /users/me | 401, '토큰 없음' | 401, '토큰 없음' | **P** |
| TC-041 | 기초 경로 | 만료된 토큰으로 API 호출 — 403 | 만료된 accessToken 보유 | 만료 토큰으로 API 호출 | 403, 토큰 만료 또는 유효하지 않음 | 403, 토큰 만료 또는 유효하지 않음 | **P** |
| TC-042 | 기초 경로 | 유효한 토큰으로 API 호출 — 성공 | 정상 로그인 상태 | 유효 토큰으로 GET /users/me | 200 OK | 200 OK | **P** |
| TC-043 | 오류 예측 | 변조된 토큰으로 API 호출 — 403 | 임의 문자열 토큰 | 변조 토큰으로 API 호출 | 403, 토큰 만료 또는 유효하지 않음 | 403, 토큰 만료 또는 유효하지 않음 | **P** |
| TC-044 | 조건 검사 | 학생 토큰으로 관리자 API 호출 | 학생 로그인 | 학생 토큰으로<br>POST /resources | 403, '관리자 권한 필요' | 403, '관리자 권한 필요' | **P** |

#### E. 공지사항 및 이슈 테스트 (적용 기법: 동등 분할, 조건 검사, 기초 경로, 오류 예측)

| TC-ID | 기법 | 테스트명 | 사전 조건 | 입력값 | 기대 결과 | 실제 결과 | P/F |
|---|---|---|---|---|---|---|---|
| TC-045 | 동등 분할 | 공지사항 목록 조회 성공 | 로그인 상태 | GET /notices | 200 OK, 최신순 목록 반환 | 200 OK, 최신순 목록 반환 | **P** |
| TC-046 | 조건 검사 | 관리자 공지사항 등록 성공 | 관리자 로그인 | POST /notices +<br>{title, content, is_urgent: true} | 201 Created | 201 Created | **P** |
| TC-047 | 조건 검사 | 학생 공지사항 등록 차단 | 학생 로그인 | POST /notices | 403 Forbidden | 403 Forbidden | **P** |
| TC-048 | 동등 분할 | 이슈 등록 성공 (학생) | 학생 로그인 | POST /issues + {title, content} | 201 Created, status: waiting | 201 Created, status: waiting | **P** |
| TC-049 | 기초 경로 | 이슈 상태 in_progress 변경 | 관리자 로그인, waiting 이슈 존재 | PATCH /issues/:id/status<br>+ {status: 'in_progress'} | 200 OK, status: in_progress | 200 OK, status: in_progress | **P** |
| TC-050 | 기초 경로 | 이슈 상태 resolved 변경 | 관리자 로그인, in_progress 이슈 존재 | PATCH /issues/:id/status<br>+ {status: 'resolved'} | 200 OK, status: resolved,<br>resolved_by 기록 | 200 OK, status: resolved,<br>resolved_by 기록 | **P** |

---

### 5-3. 테스트 결과 요약

총 50개의 테스트 케이스를 설계하여 검증을 진행하였으며, 이메일 인증 코드 발송 로직 오류(네트워크/서버 환경 설정 변수 문제)로 인한 1건의 Fail을 제외하고 총 49건의 케이스가 성공(Pass)하였다.

| 테스트 영역 | 케이스 수 | Pass | Fail | 적용 기법 |
|---|---|---|---|---|
| **인증 (TC-001 ~ TC-016)** | 16 | 15 | 1 | 동등분할, 조건검사, 경계값, 기초경로, 원인-결과 |
| **예약 (TC-017 ~ TC-033)** | 17 | 17 | 0 | 기초경로, 조건검사, 경계값, 동등분할, 원인-결과 |
| **사용자 관리 (TC-034 ~ TC-039)** | 6 | 6 | 0 | 조건검사, 동등분할, 경계값 |
| **인증 미들웨어 (TC-040 ~ TC-044)** | 5 | 5 | 0 | 기초경로, 조건검사, 오류예측 |
| **공지사항/이슈 (TC-045 ~ TC-050)** | 6 | 6 | 0 | 동등분할, 조건검사, 기초경로 |
| **합계** | **50** | **49** | **1** | **8가지 기법 전체 적용** |
...
본 프로젝트에서는 학과 내 고사양 PC 및 서버 자원을 효율적으로 공유하고 제어할 수 있는 ‘클라우드 기반 가상 실습실 예약 및 제어 센터’ 시스템을 성공적으로 설계하고 구현하였다. 

기존의 수동 관리 방식에서 벗어나 웹 인터페이스를 통해 실시간 자원 상태를 확인하고, 4단계 예약 충돌 방지 알고리즘을 적용하여 예약 프로세스의 안정성을 극대화하였다. 특히 백엔드 API 시스템 구축을 통해 이메일 인증 기반 회원가입, JWT 기반 권한 관리(학생/관리자), 공지사항 및 이슈 트래킹 기능까지 학과 실습실 운영에 필수적인 핵심 핵심 비즈니스 로직을 완벽히 통합하였다. 

또한 개발 단계에 그치지 않고, 설계된 시스템의 신뢰성을 확보하기 위해 화이트박스와 블랙박스 테스트 기법을 융합한 총 50개 항목의 정밀 테스트 케이스를 수행하였다. 비록 환경 변수 설정 문제로 인한 이메일 발송 오류 1건이 발생하였으나, 이를 제외한 모든 기능 검증(성공률 98%)을 통과함으로써 실제 학과 운영 환경에 즉시 도입 가능한 수준의 백엔드 안정성과 완성도를 확보하였음을 증명하였다.

### 6-2. 향후 과제 및 발전 방향

본 시스템의 실용성을 한 단계 더 높이기 위해 다음과 같은 고도화 작업을 향후 과제로 진행하고자 한다.

1. **실시간 양방향 알림 시스템 통합 (Websocket)**
   현재 구현된 REST API 기반의 상태 조회를 넘어, 웹소켓(Websocket) 프로토콜을 도입하여 예약 승인/거절 상태, 긴급 공지사항, 자원 반납 임박 알림 등을 사용자에게 실시간 푸시 알림으로 전달하는 기능을 추가 통합할 예정이다.

2. **클라우드 인프라 최종 배포 및 확장성 검증**
   현재 로컬 및 스테이징 환경에서 검증된 Node.js 서버와 데이터베이스 레이어를 AWS EC2 등의 퍼블릭 클라우드 인프라로 최종 이관하고, 인프라 담당 팀과의 협업을 통해 Docker 컨테이너 동적 제어 API의 실효율성과 다중 접속 환경에서의 부하 테스트를 추가로 수행할 계획이다.

3. **모니터링 데이터 기반 자원 예측 모델 도입**
   Grafana와 Prometheus를 통해 수집되는 서버 및 PC 자원의 실시간 사용량 데이터를 누적하여, 자원 활용 빈도가 높은 피크 타임을 예측하고 이에 따라 가상 컨테이너 스케줄링을 자동화하는 지능형 자원 관리 기능을 검토하고자 한다.
